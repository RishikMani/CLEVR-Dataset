...
class CnnLstmModel:
    def __init__(self, vocab, rnn_wordvec_dim=300, rnn_dim=256,
                 rnn_num_layers=2, rnn_dropout=0, cnn_res_block_dim=128,
                 cnn_num_res_blocks=0, cnn_proj_dim=512, cnn_pooling='maxpool2',
                 fc_dims=(1024,), fc_use_batchnorm=True, fc_dropout=0):
        rnn_kwargs = {
            'token_to_idx': vocab['question_token_to_idx'],
            'wordvec_dim': rnn_wordvec_dim,
            'rnn_dim': rnn_dim,
...
def build_cnn(res_block_dim=128, num_res_blocks=0, proj_dim=512,
              pooling='maxpool2'):
